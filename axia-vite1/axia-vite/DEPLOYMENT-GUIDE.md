...
> Convex backend: `https://tangible-orca-566.convex.cloud` — already running, no changes needed.
...
4. **Add Environment Variable** (important!):
   ...
   - Value: `https://tangible-orca-566.convex.cloud`
...
| Problem | Fix |
|---------|-----|
| Site says "demo mode" | Vercel Dashboard → Settings → Environment Variables → add `VITE_CONVEX_URL` = `https://tangible-orca-566.convex.cloud`, then Redeploy |
