...
export const join = mutation({
  ...
  handler: async (ctx, { email, ref }) => {
    ...
    // Validate email format and length (prevent abuse)
    if (!cleanEmail.includes("@") || !cleanEmail.includes(".") || cleanEmail.length > 254 || cleanEmail.length < 5) {
      return { success: false, error: "Please provide a valid email address" };
    }

    // Block disposable/common typo domains
    const blockedDomains = ["test.com", "example.com", "localhost"];
    ...
    // Validate referral code format to prevent injection
    if (ref && !/^AX-[A-Z0-9]{5}$/.test(ref)) {
